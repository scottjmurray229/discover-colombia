 ...
 *   2. Create project "Discover Colombia"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Colombia Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'colombia'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Colombia ${year}`,
      description: `${title}\n\nFull guide: https://discoveringcolombia.info/${slug}/`,
      tags: [`Colombia ${slug}`, 'Colombia travel', `Colombia ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Colombia ✈️ #shorts`,
      description: `${title} — Full guide: https://discoveringcolombia.info/destinations/${slug}/`,
      tags: [`${title} Colombia`, 'Colombia travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Colombia — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Colombia for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoveringcolombia.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoveringcolombia.info/companion/

#Colombia #${title.replace(/\s+/g, '')} #ColombiaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    ...
    'Colombia travel',
    'Colombia food',
    'Colombia budget travel',
    'Colombia tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Colombia`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER COLOMBIA               ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
