...
const SAMPLE_PASTE = `Day 1 - Arrive Bogota, explore La Candelaria
Day 2 - Gold Museum, Monserrate, street food tour
Day 3 - Fly to Medellin, Comuna 13 street art tour
Day 4 - Guatape day trip, El Penol rock
Day 5 - Fly to Cartagena, walled city walking tour
Day 6 - Rosario Islands boat trip, snorkeling
Day 7 - Cartagena street food, fly home`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Colombia itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We had zero signal on the boat to our Amazon lodge near Leticia and the app had everything — directions, restaurant picks, even the right Spanish phrases. Felt like traveling with a local friend."
        </div>
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'andes') return t.route.match(/Bogota|Medellin|Villa de Leyva|Salento|Barichara/i);
          if (filter === 'caribbean') return t.route.match(/Cartagena|Santa Marta|San Andres|Providencia/i);
          if (filter === 'coffee-region') return t.route.match(/Salento|Medellin|Armenia/i);
          return true;
        });
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto' as const }}>
          {['all', 'andes', 'caribbean', 'coffee-region'].map((f) => (
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'TransMilenio to Candelaria, exit at Museo del Oro' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'Spanish phrases matched to your destinations with pronunciation' },
            ...
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Colombia situation, from trike drivers to dive masters' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
