...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['cartagena', 'san-andres', 'santa-marta'],
    'nightlife': ['bogota', 'medellin', 'cali'],
    'architecture': ['cartagena', 'villa-de-leyva', 'barichara'],
    'history': ['cartagena', 'bogota', 'villa-de-leyva'],
    'nature': ['leticia', 'salento', 'santa-marta'],
    'food': ['bogota', 'cartagena', 'cali'],
    'coffee': ['salento', 'medellin', 'bogota'],
    'budget': ['salento', 'barichara', 'leticia'],
    'backpacker': ['medellin', 'salento', 'santa-marta'],
    'carnival': ['cartagena', 'cali', 'bogota'],
    'culture': ['bogota', 'cartagena', 'cali'],
    'hiking': ['santa-marta', 'salento', 'barichara'],
    'amazon': ['leticia'],
    'diving': ['san-andres', 'providencia', 'santa-marta'],
    'romantic': ['cartagena', 'barichara', 'providencia'],
    'adventure': ['santa-marta', 'leticia', 'tatacoa-desert'],
    'andean': ['bogota', 'medellin', 'villa-de-leyva', 'salento', 'barichara', 'tatacoa-desert'],
    'caribbean': ['cartagena', 'santa-marta', 'san-andres', 'providencia'],
    'pacific': ['cali'],
    'amazon-orinoco': ['leticia'],
  };
  ...
  return ['bogota', 'medellin', 'cartagena'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discovering Colombia AI Trip Planner. You are an expert on travel across Colombia — from the colonial walls of Cartagena to the Amazon rainforest of Leticia.

RULES:
- All prices in BOTH COP and USD ($). Use rate: COP 4,000 = $1 USD
...
- Colombia uses a single currency: the Colombian Peso (COP). Express all prices in COP for consistency
...
{
  "title": "string — trip title like '7-Day Colombia Adventure'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "brl": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Bogota'",
      "destination": "string — slug like 'bogota'",
      "items": [
        {
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Colombia trip:\n${parts.join('\n')}`;
